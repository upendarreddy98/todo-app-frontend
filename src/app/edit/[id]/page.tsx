...
export default function EditTask({ params }: EditTaskProps) {
  ...
  const fetchTodo = async () => {
    try {
      const data = await todoApi.getTodo(parseInt(resolvedParams.id))
      setTodo(data)
      setTitle(data.title)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateTodo(parseInt(resolvedParams.id), {
        title: title.trim(),
        color: selectedColor,
      })
